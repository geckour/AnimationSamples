...
class PlayStoreTransition : Transition {
    ...
    private fun captureValues(transitionValues: TransitionValues) {
        ...
        transitionValues.values["position"] = position
    ...
    override fun captureStartValues(transitionValues: TransitionValues) {
        ...
        view.draw(canvas)
        transitionValues.values["image"] = bitmap
    }
    ...
    override fun createAnimator(sceneRoot: ViewGroup, startValues: TransitionValues?,
        ...
        val startBounds = startValues.values.getOrDefault("bounds", null) as Rect?
        val endBounds = endValues.values.getOrDefault("bounds", null) as Rect?
        ...
        val startImage = startValues.values.getOrDefault("image", null) as Bitmap? ?: return null

        val startView = addViewToOverlay(sceneRoot, startImage.width, startImage.height,
                BitmapDrawable(context.resources, startImage))
        ...
        //保存しておいた位置
        val startPosition = startValues.values.getOrDefault("position", null) as IntArray?
        val endPosition = endValues.values.getOrDefault("position", null) as IntArray?
        ...
        //保存値から現在値を引いて差分を計算
        val transStartX = startPosition[0] - sceneRootPosition[0]
        val transStartY = startPosition[1] - sceneRootPosition[1]
        val transEndX = endPosition[0] + (endView.width - startView.width) / 2
        val transEndY = endPosition[1] + (endView.height - startView.height) / 2

        //計算結果をviewに適用
        startView.translationX = transStartX.toFloat()
        startView.translationY = transStartY.toFloat()
        startBallView.translationX = transStartX.toFloat()
        startBallView.translationY = transStartY.toFloat()
        ...
        val revealStartView = createCircularReveal(startView, calculateMaxRadius(startView),
                Math.min(calculateMinRadius(startView) / 2, calculateMinRadius(endView) / 2).toFloat())
        ...
        val fadeInStartBallView = ObjectAnimator.ofFloat<View>(startBallView, View.ALPHA, 0f, 1f)
        val fadeInEndView = ObjectAnimator.ofFloat<View>(endView, View.ALPHA, 0f, 1f)
        val fadeOutEndBallView = ObjectAnimator.ofFloat<View>(endBallView, View.ALPHA, 1f, 0f)
        ...
        val moveStartView = ObjectAnimator.ofFloat(startView, View.TRANSLATION_X, View.TRANSLATION_Y,
                pathMotion.getPath(transStartX.toFloat(), transStartY.toFloat(), transEndX.toFloat(), transEndY.toFloat()))
    ...
    companion object {
        ...
        //viewに対する内接円の半径を計算する
        internal fun calculateMinRadius(view: View): Int {
